...
def macd_cross_only_buy_or_sell_strategy(data, data_w, signal, initial_balance=10000, holding_days=7, \
    ...
    out_results['长期持有收益']=(df['close_price'].values[-1]-df['open_price'].values[0])/df['open_price'].values[0]
    out_results['策略/持有收益比']=out_results['收益率']/out_results['长期持有收益']
    if out_results['收益率']-out_results['长期持有收益']>0:
        out_results['跑赢长期持有']=True
    else:
        out_results['跑赢长期持有']=False
...
def dump_results(dump_file,res):
    desired_index_order_CHN = ['币对', '胜率', '盈亏比','最大回撤','年化收益率',
                           '收益率','长期持有收益','策略/持有收益比','跑赢长期持有']
    desired_index_order = [
        'Currency Pair',
        'Win Rate',
        'Risk-Reward Ratio',
        ...
        'Strategy/Holding Return Ratio',
        'Outperform Long-Term Holding'
    ]
    index_mapping = {
    '币对': 'Currency Pair',
    '胜率': 'Win Rate',
    ...
    '收益率': 'Return Rate',
    '长期持有收益': 'Long-Term Holding Return',
    '策略/持有收益比': 'Strategy/Holding Return Ratio',
    '跑赢长期持有': 'Outperform Long-Term Holding'
    }
    # 创建一个空的DataFrame，列名是字典的键
    df_res = pd.DataFrame(columns=desired_index_order)

    # 填充数据
    for index in desired_index_order_CHN:
        if index in ['币对','跑赢长期持有']:
            df_res.loc[0, index_mapping[index]] = res[index]
        else:
            df_res.loc[0, index_mapping[index]] = "%.5f" % res[index]
    try:
        pd.read_csv(dump_file)
        write_header = False
    except FileNotFoundError:
        write_header = True

    # 使用to_csv函数以追加模式写入数据
    df_res.to_csv(dump_file, mode='a', header=write_header, index=False)
    return
...
for internal in internals:
    for symbol in symbols:
        try:
            ...
            # if not os.path.exists(f"D:\\Trade\\Lorentzian\\data\\{symbol}_{internal}_spot.csv"):
            #     print('Not found! %s'%(f"D:\\Trade\\Lorentzian\\data\\{symbol}_{internal}_spot.csv"))
            df = pd.read_csv(f"D:\\Trade\\Lorentzian\\data\\{symbol}_{internal}_spot.csv")
            output_fig=f"D:\\Trade\\Lorentzian\\output\\{symbol}_{internal}_{num_candles}.png"
            output_csv=f"D:\\Trade\\Lorentzian\\output\\stat_{internal}_{num_candles}.csv"
            if symbol=='BTCUSDT' and os.path.exists(output_csv):
                os.remove(output_csv) # remove initially
            df = df[-num_candles:]
            ...
            out_results=macd_cross_only_buy_or_sell_strategy(ma_data, None, "signal", initial_balance=10000, holding_days=10000,
                                                 bear_mode=False, transaction_fee=0.0,stop_loss=-1,save_fig=output_fig)
            out_results['币对'] ="%s_%s"%(symbol,internal)
            dump_results(output_csv,out_results)
        except:
            print('skipping %s %s'%(symbol,internal))
# # for 15 min
# for symbol in symbols:
#     for internal in ['15m']:
#         try:
