...
class LorentzianClassification:
    ...
    def series_from(data: pd.DataFrame, feature_string, f_paramA, f_paramB) -> np.array:
        match feature_string:
            ...
            case "VOL":
                return ml.n_vol(data['volume'],0,0)


    def __init__(self, data: pd.DataFrame, features: list = None, settings: Settings = None, filterSettings: FilterSettings = None,num_feat=5):
        self.df = data.copy()
        self.features = []
        ...
        self.filter = Filter(
            volatility = ml.filter_volatility(data['high'], data['low'], data['close'], filterSettings.useVolatilityFilter, 1, 10), # 计算近期ATR相比于较长期ATR的波动性
    ...
    def plot(self, name: str):
        ...
        self.yhat1=pd.DataFrame(self.yhat1)
        self.yhat2 = pd.DataFrame(self.yhat2)
        sub_plots = [
            ...
            mpf.make_addplot(self.df["startLongTrade"], ylabel="startLongTrade", color='green', type='scatter', markersize=500, marker='^'),
            mpf.make_addplot(self.df["startShortTrade"], ylabel="startShortTrade", color='red', type='scatter', markersize=500, marker='v'),
            # mpf.make_addplot(self.df["endLongTrade"], ylabel="endLongTrade", color='yellow', type='scatter',
            #                  markersize=500, marker='^'),
            # mpf.make_addplot(self.df["endShortTrade"], ylabel="endShortTrade", color='blue', type='scatter',
            #                  markersize=500, marker='v'),
        ]
        ...
        fig, axlist = mpf.plot(self.df[['open', 'high', 'low', 'close']].head(len), type='candle', style=s, addplot=sub_plots, figsize=(70,40) ,returnfig=True)
