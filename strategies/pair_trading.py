...
def pair_trading(df1,df2,symbols,spread_type='simple'):
    # 假设我们有两个加密货币的价格数据，以Pandas Series的形式给出
    # 这里使用随机数据作为示例
    crypto1=(df1['close_price']).to_numpy()
    crypto2 = (df2['close_price']).to_numpy()
    ...
    # 计算SMA
    window_size = 20  # SMA窗口大小，可以根据需要调整
    sma = pd.Series(returns_crypto1).rolling(window=window_size).mean() - pd.Series(returns_crypto2).rolling(window=window_size).mean()
...
if __name__ == '__main__':
    ...
    df1 = pd.read_csv(f"D:\\Trade\\data\\{symbols[0]}_{internal}_spot.csv")
    df2 = pd.read_csv(f"D:\\Trade\\data\\{symbols[1]}_{internal}_spot.csv")
    ...
    dump_file='D:\\Trade\\results\\pair_trading\\24.8.28\\'
